...
router.get("/", async (req: Request, res: Response) => {
  ...
  const mapData: QueryResult = await pool.query(
    "SELECT maps.mapname, maps.map_id, array_agg(CASE WHEN l.playername IS NULL THEN NULL ELSE jsonb_build_object('playername', l.playername, 'timer', l.timer, 'seconds', l.seconds) END) as leaderData FROM maps left join lateral (SELECT map_id, playername, timer, seconds FROM leaderboard WHERE maps.map_id = map_id ORDER BY seconds, created_at LIMIT 5) as l ON l.map_id = maps.map_id GROUP BY maps.mapname, maps.map_id"
  );
...
router.post("/", async (req: Request, res: Response) => {
  ...
  await pool.query(
    "INSERT INTO leaderboard (playername, seconds, timer, map_id, created_at) VALUES ($1, $2, $3, $4)",
